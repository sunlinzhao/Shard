...
path = 'D:\\MyProject\\Python\\Shard\\log\\' + 'mylog - shard_two_pbft_with_sync - ' + name + '.log'
...
def draw(shards, interval, num_stop):
    ...
    # logger.info(f'>>>>>>>>>>>>>>>>>>>>> consensus_trans result is : {y3} <<<<<<<<<<<<<<<<<<<')
    print(f'time_consensus: {config.time_consensus}')
    logger.info(f'>>>>>>>>>>>>>>>>>>>>> time_consensus result is : {config.time_consensus} <<<<<<<<<<<<<<<<<<<')
    for i, shard in enumerate(shards):
        logger.info(f'>>>>>>>>>>>>>>>>>>>>> shard {i} time_consensus result is : {shard.time_consensus} <<<<<<<<<<<<<<<<<<<')

    commit_time_min = min(shard.time_consensus)
    confirm_time_min = min(config.time_consensus)
    latency = confirm_time_min - commit_time_min
